...
install.pinstall.packages("CircStats")
install.packages("boot")
install.packages("MASS")
...
steplen <-earth.dist(long1=whaledivestats$beglong,lat1=whaledivestats$beglat,long2=whaledivestats$endlong,lat2=whaledivestats$endlat)
steplen <- steplen * 1000
summary(steplen)
whaledivestats$step <- steplen
...
write.csv(data, file = "whale_data_cleaned_step.csv")
