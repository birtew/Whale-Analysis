...
whaledata <- read.csv("dive_stats_170921.csv")
...
pn2pw <- function(mu1, mu2, mu3, sigma1, sigma2, sigma3, gamma){   #gamma as vector of offdiagonals
  for(i in 1:3){
  ...
  tgamma <- qlogis(gamma)
  parvect <- c(tmu1, tmu2, tmu3,tsigma1, tsigma2, tsigma3, tgamma)
  return(parvect)
}
...
pw2pn <- function(parvect,N){     # parvect contains of mu, sigma as above, gamma
  ...
  sigma1 <- exp(c(parvect[(1+3*N):(4*N)]))
  sigma2 <- exp(c(parvect[(1+4*N):(5*N)]))
  sigma3 <- exp(c(parvect[(1+5*N):(6*N)]))
  gamma <- diag(N) 
  if(N > 1){
    gamma[!gamma] <- exp(parvect[(6*N+1):(7*N+N*(N-1))])
...
mle <- function(obs, mu01, mu02, mu03, sigma01, sigma02, sigma03, gamma0, N){
  parvect <- pn2pw(mu01, mu02, mu03, sigma01, sigma02, sigma03, gamma0)
...
fitmult <- function(obs, n_fits, N){
  modl <- list()
  for (i in 1:n_fits){
    modl[[i]] <- mle(obs, mu01, mu02, mu03, sigma01, sigma02, sigma03, gamma0, N)
  }
  return(modl)
}              
...
###########################
# 2 states
whale_dive_mod2 <- mle(obs, c(30, 200), c(20, 80), c(100, 600),
      c(25, 70), c(10, 20), c(50, 90), c(0.9, 0.8), 2)

# 3 states
whale_dive_mod3 <- mle(obs, c(30, 150, 250), c(20, 80, 110), c(50, 100, 600),
    c(25, 60, 70), c(10, 20, 30), c(30, 90, 100), c(0.5, 0.4, 0.3, 0.2, 0.25, 0.1), 3)


## zufällig gezogene Startwerte:
# 2 states
whale_dive_rmod2 <- mle(obs, c(runif(2, 1, 250)), c(runif(2, 1, 120)), c(runif(2, 1, 600)),
